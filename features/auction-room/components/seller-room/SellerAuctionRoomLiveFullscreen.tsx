...
type SellerAuctionRoomLiveFullscreenProps = {
  open: boolean;
  onClose: () => void;
...
export function SellerAuctionRoomLiveFullscreen({
  open,
  onClose,
  videoStream,
  isLive,
  ...
  heroImageUrl,
  auctionTitle,
}: SellerAuctionRoomLiveFullscreenProps) {
  useEffect(() => {
    if (!open) return;
        ...
        <div className="absolute inset-0 bg-black">
          {videoStream ? (
            ...
            <div className="flex size-full flex-col items-center justify-center gap-2 text-zinc-400">
              <VideoOff className="size-12 opacity-50" />
            ...
            {isHostProducer ? (
              <>
                <button
                  ...
                  className={cn(
                    'flex size-9 items-center justify-center rounded-lg hover:bg-white/10',
